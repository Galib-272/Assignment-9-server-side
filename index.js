...
app.put("/comments/:id", verifyEcosystemToken, async (req, res) => {
  try {
    ...
    if (!text || !text.trim()) {
      return res.status(400).json({ message: "Comment body vector text missing." });
    }
    ...
    const existingComment = await db.collection("comments").findOne(databaseQuery);
    if (!existingComment) {
      return res.status(404).json({ message: "Target comment metric node missing." });
    }

    if (existingComment.userEmail !== req.decodedIdentity.email) {
      return res.status(403).json({ message: "Forbidden operational modification sequence." });
    }

    const updateResult = await db.collection("comments").updateOne(databaseQuery, {
      $set: {
        text: text.trim(),
        timestampRaw: new Date().toISOString()
      }
...
app.delete("/comments/:id", verifyEcosystemToken, async (req, res) => {
  try {
    ...
    if (existingComment.userEmail !== req.decodedIdentity.email) {
      return res.status(403).json({ message: "Forbidden operational deletion sequence." });
    }
